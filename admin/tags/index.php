...
if(!isset($_REQUEST['orderby'])){$orderby="nom_tag ASC";}else{$orderby=$_REQUEST['orderby'];}
...
if($recherche=="")
	{
	 $sql = "SELECT * FROM tags ORDER BY ".$orderby." LIMIT ".$limite."";
		  $sql_num = "SELECT id_tags FROM tags ORDER BY ".$orderby."";
			
    } else
			{
			$sql = "SELECT * FROM tags WHERE nom_tag LIKE '%".$recherche."%' ORDER BY ".$orderby." ";
		 	$sql_num = "SELECT id_tags FROM tags WHERE nom_tag LIKE '%".$recherche."%'  ORDER BY ".$orderby."";
...
<div class="header icon-48-article"> Gestion des mots-clés</div>
<div class="clr"></div>
</div>
<div class="b">
<div class="b">
...
<div class="t"></div>
			...
			<thead>
				<tr>
					...
					<th width="28">&nbsp;</th>
					<th width="500" class="title">	<a href="#">Mots-clés</a></th>
					<th width="100" class="title">	<a href="#">Hits</a></th>
...
{
$id_tags=$a_requete["id_tags"];
$nom_tag=stripslashes($a_requete["nom_tag"]);
$hits_tag=$a_requete["hits_tag"];
...
?>			
					...
					<td><?php echo $i; ?></td>
					<td><input type="radio" name="id_tags" id="id_tags" value="<?php echo $id_tags;?>"><label for="yy"></label></td>
					<td><a href="edit.php?id_tags=<?php echo $id_tags;?>"><?php echo $nom_tag;?></a></td>
					<td><?php echo $hits_tag;?></td>
...
for(i=0;i<document.adminForm.id_tags.length;i++)
{
if(document.adminForm.id_tags[i].checked == true)
{var cochee = 1; break;}
else {var cochee = 0;}
}
if(cochee == 0)
